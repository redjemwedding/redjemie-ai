...
// ── CTO Logo SVG ──────────────────────────────────────────────────
function CTOLogo({ size = 52, dark = false }) {
  return (
...
// ── Register cert in Firestore so verify link works ───────────────
async function registerCertificate({ certId, uid, courseId, course, profile, completedAt }) {
  try {
...
export default function CertificatePage() {
  ...
  async function downloadPDF() {
    ...
    try {
      ...
      const canvas = await window.html2canvas(certRef.current, { scale:3, useCORS:true, allowTaint:true, backgroundColor:'#FFFFF8' })
  ...
  return (
    <div className="max-w-screen-lg mx-auto px-4">
      ...
      {/* ════════════════════════════════════════════════════
          ...
          Inspired by Harvard, Oxford, professional diplomas
      ════════════════════════════════════════════════════ */}
      <div ref={certRef} style={{
        ...
        <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center', pointerEvents:'none', overflow:'hidden' }}>
          <div style={{ fontSize:'130px', fontWeight:'900', color:'rgba(180,140,60,0.04)', letterSpacing:'-2px', userSelect:'none', fontFamily:'Georgia,serif', transform:'rotate(-10deg)', whiteSpace:'nowrap' }}>
            CERTIFIED
          </div>
        </div>
        ...
        <div style={{ position:'relative', zIndex:1, height:'100%', display:'flex', flexDirection:'column', padding:'24px 52px 20px' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'10px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
              <CTOLogo size={44} />
              <div>
                <div style={{ fontFamily:'Montserrat,sans-serif', fontWeight:'900', fontSize:'10.5px', letterSpacing:'3px', color:'#1a1a1a', textTransform:'uppercase' }}>
                  CTO Access Forum University
                </div>
                <div style={{ fontFamily:'Montserrat,sans-serif', fontSize:'7.5px', letterSpacing:'2px', color:'#6B5A2A', textTransform:'uppercase', marginTop:'1px' }}>
                  university.redjemie.com · Dubai, United Arab Emirates
                </div>
              </div>
            </div>
            <div style={{ textAlign:'right' }}>
              <div style={{ fontFamily:'Montserrat,sans-serif', fontSize:'7px', letterSpacing:'2px', color:'#8B7340', textTransform:'uppercase', marginBottom:'2px' }}>
                Official Academic Credential
              </div>
              <div style={{ fontFamily:'Montserrat,sans-serif', fontSize:'7px', color:'#9A8A6A', letterSpacing:'0.5px' }}>
                Ref: {certId}
              </div>
            </div>
          </div>
          ...
          <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', textAlign:'center' }}>
            ...
            <div style={{ fontFamily:'Montserrat,sans-serif', fontWeight:'400', fontSize:'8.5px', letterSpacing:'5px', color:'#8B7340', textTransform:'uppercase', marginBottom:'5px' }}>
              Certificate of Achievement
            </div>
            <div style={{ fontFamily:'Georgia,serif', fontStyle:'italic', fontSize:'10px', color:'#5A4A2A', marginBottom:'10px' }}>
              This is to certify that
            </div>

            {/* RECIPIENT NAME */}
            <div style={{ fontFamily:'Georgia,"Palatino Linotype",serif', fontSize:'40px', color:'#1A1A1A', letterSpacing:'1px', marginBottom:'2px', lineHeight:'1', fontWeight:'400' }}>
              {profile?.displayName}
            </div>
            {profile?.title && (
              <div style={{ fontFamily:'Georgia,serif', fontStyle:'italic', fontSize:'11px', color:'#6B5A2A', marginBottom:'4px' }}>{profile.title}</div>
            )}
            ...
            <div style={{ fontFamily:'Georgia,serif', fontStyle:'italic', fontSize:'10px', color:'#5A4A2A', marginBottom:'9px' }}>
              has successfully completed
            </div>

            {/* COURSE TITLE */}
            <div style={{ fontFamily:'Georgia,"Palatino Linotype",serif', fontSize:'20px', color:'#1A1A1A', maxWidth:'560px', lineHeight:'1.35', marginBottom:'7px', fontWeight:'400' }}>
              {course?.title}
            </div>
          ...
          {/* divider */}
          <div style={{ display:'flex', height:'1px', margin:'12px 0 10px' }}>
            <div style={{ flex:1, background:'linear-gradient(90deg, transparent, #C8B87A, #8B7340, #C8B87A, transparent)' }} />
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr auto 1fr', gap:'20px', alignItems:'flex-end', marginBottom:'10px' }}>
            ...
            <div style={{ textAlign:'center' }}>
              <div style={{ fontFamily:'Georgia,serif', fontStyle:'italic', fontSize:'11.5px', color:'#3A3020', marginBottom:'5px' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', paddingTop:'8px', borderTop:'0.5px solid rgba(180,150,60,0.2)' }}>
            ...
            {/* validity */}
            <div style={{ textAlign:'right' }}>
              <div style={{ fontFamily:'Montserrat,sans-serif', fontSize:'6px', color:'#9A8A6A', letterSpacing:'0.5px' }}>Valid until {expiryStr}</div>
      ...
      <div className="mt-3 bg-[#0d0d0d] border border-white/[.04] rounded-[10px] p-4">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          {[
            { t:'Cryptographically Verified', d:'SHA-256 signed unique ID. Any modification instantly invalidates the certificate.' },
            { t:'Publicly Verifiable',        d:'Anyone can verify at university.redjemie.com/verify — employers, LinkedIn, clients.' },
            { t:'Print Ready',                d:'Download as A4 PDF. Suitable for framing, LinkedIn, and professional portfolios.' },
          ].map(c => (
            <div key={c.t}>

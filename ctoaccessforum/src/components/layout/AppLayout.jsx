  ...
  const NavItem = ({ to, label }) => (
    <NavLink to={to} onClick={() => setOpen(false)}
      className={({ isActive }) =>
        ...
        ${isActive
          ? 'bg-[rgba(229,24,27,.1)] text-[#FF4447] font-semibold'
          : 'text-gray-500 hover:bg-white/[.04] hover:text-white'}`}>
      {label}
    </NavLink>
  )
        ...
        <div className="flex items-center gap-3 ml-auto flex-shrink-0">
          {onlineUsers.length > 0 && (
            <div className="hidden md:flex items-center gap-1.5">
              ...
              <span className="text-[0.67rem] text-gray-400 font-[Montserrat]">
          ...
          {CHANNELS.map(c => (
            <NavLink key={c.to} to={c.to} onClick={() => setOpen(false)}
              className={({ isActive }) =>
                ...
                ${isActive
                  ? 'bg-[rgba(229,24,27,.08)] text-[#FF4447]'
                  : 'text-gray-500 hover:bg-white/[.03] hover:text-white'}`}>
        ...
        <aside className="hidden xl:flex flex-col w-[240px] flex-shrink-0 border-l border-white/[.05] bg-[#0c0c0c] p-4 gap-4 sticky top-14 h-[calc(100vh-3.5rem)] overflow-y-auto">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <span className="text-[0.65rem] font-bold tracking-[.1em] uppercase text-gray-600 font-[Montserrat]">
                Online Now
              </span>
            ...
            {onlineUsers.length === 0 ? (
              <p className="text-[0.7rem] text-gray-700">No other members online right now.</p>
            ) : onlineUsers.map(u => (
              <div key={u.id} className="flex items-center gap-2.5 py-1.5 border-b border-white/[.03] last:border-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[0.6rem] text-gray-600 truncate">
                    {u.title || ROLE_META[u.role]?.label || 'Member'}
                  </div>
                </div>
              </div>
            ))}

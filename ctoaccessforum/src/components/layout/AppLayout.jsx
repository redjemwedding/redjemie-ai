...
const CHANNELS = [
  ...
  { to: 'forum/vendors',    label: 'Vendor Reviews',  dot: '#6366f1' },
  { to: 'forum/career',     label: 'Career Advice',   dot: '#ec4899' },
]
...
export default function AppLayout({ children }) {
  ...
  useEffect(() => {
    ...
    const ping = async () => { try { await updateDoc(ref, { lastSeen: serverTimestamp(), isOnline: true }) } catch (_) {} }
    ping()
  ...
  useEffect(() => {
    if (!profile?.uid) return
    const q = query(collection(db, 'users', profile.uid, 'notifications'), where('read', '==', false))
    return onSnapshot(q, snap => setUnreadCount(snap.size), () => {})
  }, [profile?.uid])

  // close sidebar on nav (mobile)
  function navTo(path) { nav(path); setOpen(false) }

  const NavItem = ({ to, label }) => (
    <NavLink to={to} onClick={() => setOpen(false)}
      className={({ isActive }) =>
        `flex items-center px-3 py-2.5 rounded-[8px] text-[0.78rem] font-medium transition-all
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#080808]">
      ...
      {/* ── TOP BAR ── */}
      <header className="sticky top-0 z-[300] h-14 flex items-center gap-3 px-3 sm:px-5 bg-[rgba(8,8,8,.98)] backdrop-blur-2xl border-b border-white/[.06]">

        {/* hamburger */}
        <button
          className="lg:hidden p-2 rounded-[8px] bg-white/[.04] border border-white/[.06] text-gray-400 flex-shrink-0"
        ...
        <div className="flex items-center gap-2 ml-auto flex-shrink-0">
          ...
          {onlineUsers.length > 0 && (
            <div className="hidden md:flex items-center gap-1.5">
              ...
              <span className="text-[0.65rem] text-gray-500 font-[Montserrat]">{onlineUsers.length} online</span>
            </div>
          )}
          ...
          <button onClick={() => navTo('profile')}
            className="w-8 h-8 rounded-full flex items-center justify-center font-bold font-[Montserrat] text-white text-[0.6rem] flex-shrink-0 ring-1 ring-white/10 hover:ring-red-500/40 transition-all"
            style={{ background: color }}>
            {av}
          </button>

          <button onClick={signOut}
            className="hidden md:block text-[0.7rem] text-gray-600 hover:text-red-400 transition-colors font-[Montserrat]">
            Sign out
          </button>
        </div>
      </header>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <aside className={`
          ...
          flex-col w-[220px] flex-shrink-0
          border-r border-white/[.05] bg-[#0c0c0c]
          fixed lg:sticky top-14
        ...
        `}>
          ...
          {CHANNELS.map(c => (
            <NavLink key={c.to} to={c.to} onClick={() => setOpen(false)}
              className={({ isActive }) =>
                `flex items-center gap-2.5 px-3 py-2 rounded-[8px] text-[0.75rem] font-medium transition-all
          ...
          <div className="mt-auto pt-3 border-t border-white/[.05]">
            <div className="flex items-center gap-2.5 px-3 py-2">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[0.76rem] font-semibold truncate">{profile?.displayName}</div>
            ...
            <button onClick={signOut}
              className="w-full text-left px-3 py-2 rounded-[8px] text-[0.75rem] text-gray-600 hover:text-red-400 hover:bg-red-900/10 transition-all font-[Montserrat]">
              Sign out
            </button>
          </div>
        </aside>
        ...
        {/* overlay */}
        {open && (
          <div className="fixed inset-0 bg-black/70 z-[199] lg:hidden" onClick={() => setOpen(false)} />
        )}
        ...
        <aside className="hidden xl:flex flex-col w-[220px] flex-shrink-0 border-l border-white/[.05] bg-[#0c0c0c] p-4 gap-4 sticky top-14 h-[calc(100vh-3.5rem)] overflow-y-auto">
          <div>
            <div className="flex items-center justify-between mb-3">
              <span className="text-[0.63rem] font-bold tracking-[.1em] uppercase text-gray-600 font-[Montserrat]">Online Now</span>
              <div className="flex items-center gap-1.5">
                <span className="w-1.5 h-1.5 rounded-full bg-green-400 animate-pulse"/>
                <span className="text-[0.63rem] text-gray-500 font-[Montserrat]">{onlineUsers.length}</span>
              </div>
            </div>
            {onlineUsers.length === 0 ? (
          ...
          {profile && (
            <div>
              <div className="text-[0.63rem] font-bold tracking-[.1em] uppercase text-gray-600 font-[Montserrat] mb-3">My Stats</div>
              {[
